...
def analyze_beat(audio_path, penalty=1000, return_beat=False):
    num_windows = 21
...
def get_magnitude(fourier):
    ...
    return magnitude*1.5 + magnitude2*0.75 + magnitude3*0.75 + magnitude4*0.75
...
def analyze_activation(beat_activation, num_windows=21, num_ensemble=3, sr=44100, fps=50, penalty=200, fourier_step=500):
    ...
    for i, idx_list in enumerate(combined_segments):
        ...
        final_segments.append({'time': time_, 'bpm': bpm_, 'offset': offset_, 'beat':beats_, 'beat_offset':beat_offsets_, 'fourier':fourier_, 'idx':sorted(idx_list)})
